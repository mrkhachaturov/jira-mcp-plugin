 ...
 *
 * <p>Atlassian's plugin framework hardcodes {@code asyncSupported=false} on
 * {@code <servlet>} modules: {@code ServletModuleDescriptor} does not override
 * the base class's {@code getDefaultAsyncSupported()} (verified via javap, base
 * returns {@code iconst_0}). There is no XML attribute or system property to
 * enable async on a plugin {@code <servlet>} — servlets registered that way
 * are structurally async-unsupported.
 *
 * <p>{@code <servlet-filter>} modules, in contrast, honor the JVM property
 * {@code atlassian.plugins.filter.async.default=true} (read via
 * {@code Boolean.getBoolean} in {@code ServletFilterModuleDescriptor}'s static
 * init). When that flag is set system-wide, plugin filters become
 * async-supported and can call {@code request.startAsync()}.
 *
 * <p>This filter owns its URL pattern ({@code /plugins/servlet/mcp}), calls
 * the SDK transport servlet's {@link HttpServlet#service(ServletRequest, ServletResponse)}
 * method directly, and never invokes {@link FilterChain#doFilter} — it IS the
 * endpoint, not a wrapper around one. The SDK's transport calls
