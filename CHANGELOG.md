...
- **Every tool declares its parameters as a record, and the advertised JSON Schema is derived from it.** A tool extends `TypedTool<A>`; `inputSchema()` and the `execute` overloads are final, so a parameter cannot be advertised without being readable, and reading one that was never declared does not compile. Jackson binds the arguments, which brings snake-case wire names and value coercion, and rejects unknown keys — every schema now carries `additionalProperties: false`, so a misspelled parameter is refused instead of silently dropped. Auth and progress moved out of the method signatures into `McpContext`, leaving one `run` body whether or not the caller asked for progress notifications. The contract is in [tool-authoring.md](wiki/project/tool-authoring.md).
- **Parameters now declare the type they always meant.** Lists that travelled as comma-separated strings are arrays (`components`, `projects_filter`, `issue_keys`, `issue_ids_or_keys`, `metrics`, `state`), objects that travelled as strings holding JSON are objects (`additional_fields`, `fields` on `update_issue` and `transition_issue`, `comment_visibility`, `visibility`, `answers`), and repeated objects described only in prose are records with their own schema (`batch_create_issues.issues`, `batch_create_versions.versions`). Numeric Jira ids are integers (`board_id`, `sprint_id`, `link_id`, `comment_id`, `transition_id`, `service_desk_id`, `queue_id`). A compliant client reads the new schema from `tools/list` each session and adapts; a hardcoded caller does not.
- **Parameters renamed or removed.** `batch_get_changelogs.fields` → `changed_fields` (it filtered changelog entries, while `fields` elsewhere is a selector Jira parses); `remove_watcher.username` → `user_identifier` (its two neighbours already used that name). Removed: `update_issue.additional_fields` (indistinguishable from `fields`), `add_worklog.original_estimate` (not a worklog field — it lives on the issue, reachable through `update_issue`), `search_fields.refresh` (never read; a real implementation needs per-user caching). Each is now refused as an unknown parameter rather than ignored.
...
- **Values outside a documented set are refused before the call.** `update_sprint.state`, `get_issue_sla.metrics` and the development-info `data_type` named their permitted values in prose and enforced none of them; `get_issue_sla` silently dropped an unrecognised metric and returned an empty result.
- **`get_board_issues` no longer sends `expand=version`** on every request — a default its own description never mentioned and the agile endpoint does not define.
- **`batch_get_changelogs` built its JSON by string concatenation**, so a quote in an issue key or an error message produced invalid JSON.
- **`get_issue_development_info` no longer returns four permanently empty arrays** (`pullRequests`, `branches`, `commits`, `repositories`) that nothing ever populated.
- **`batch_create_versions` forwarded any key the caller invented** straight to Jira, while its single-version sibling accepted only the five it declares. Both now build the body the same way.
