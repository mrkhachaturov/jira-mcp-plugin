 ...
 * {@code Content-Length} header. A fixed-length header over the cap is rejected as a fast
 * path; otherwise the stream is drained up to the cap + 1 byte and, if more remains, the
 * request is rejected with 413. This closes the bypass where a client omits Content-Length
 * (chunked transfer encoding) or under-declares it to slip an oversized body past the gate.
 * The already-read bytes are re-wrapped in a {@link BufferedRequestWrapper} so the SDK
 * transport downstream still reads the body intact.
 */
@UnrestrictedAccess
