 ...
 *
 * <p>Exact string match, except for loopback URIs, where the port is ignored per RFC 8252 §7.3 —
 * a native client on an ephemeral port declares {@code http://localhost/callback} and sends
 * {@code http://localhost:54321/callback}. Scheme, host, path and query must still match exactly
 * ({@code localhost} is not {@code 127.0.0.1}). Safe because a loopback address is only reachable
