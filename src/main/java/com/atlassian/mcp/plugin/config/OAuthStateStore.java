...
public class OAuthStateStore {
  ...
  public String createPendingAuth(
      String clientRedirectUri,
      String clientState,
      String codeChallenge,
      String codeChallengeMethod,
    ...
    pendingAuths.put(
        internalState,
        new PendingAuth(
            clientRedirectUri, clientState, codeChallenge, codeChallengeMethod, clientId));
  ...
  public RegisteredClient registerClient(String clientName, List<String> redirectUris) {
    ...
    if (registeredClients.size() >= MAX_REGISTERED_CLIENTS) {
      log.warn(
          "[MCP-SEC] Client registration capacity reached ({}), rejecting", MAX_REGISTERED_CLIENTS);
      return null;
    }
