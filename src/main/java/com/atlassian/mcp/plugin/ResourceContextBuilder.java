 ...
 *
 * <p>Extracted verbatim from {@code JsonRpcHandler.buildStructuredContent} and {@code
 * JsonRpcHandler.normalizeIssue} (deleted in Task 2 Step 2.10). Lives in {@link
 * com.atlassian.mcp.plugin} (not {@code rest}) because Task 3 will hook it from {@code
 * McpToolAdapter} for SDK-driven structured content emission.
 *
 * <p>UI-linked tools (verified from {@code JsonRpcHandler.UI_TOOLS} before deletion):
 *
 * <ul>
 *   <li>{@code get_issue} — single-issue shape (object at root)
 *   <li>{@code search} — list shape ({@code issues[]}, {@code total})
 *   <li>{@code get_project_issues} — list shape
 *   <li>{@code get_board_issues} — list shape
 *   <li>{@code get_sprint_issues} — list shape
 * </ul>
 *
 ...
 * }</pre>
 *
 * <p>Note (per Codex R2-F4): even single-issue tools produce {@code issues[]} + {@code totalCount};
 * the widget reuses one component for both single + list rendering.
 */
...
public class ResourceContextBuilder {
  ...
  /**
   * Normalize a single Jira issue node into the structuredContent issue schema. Returns null if the
   * node does not look like a Jira issue.
   *
  ...
  private ObjectNode normalizeIssue(JsonNode issue) {
    ...
    if (status != null && status.isObject()) {
      ...
      if (statusCategory != null && statusCategory.isObject()) {
        statusOut.put(
            "category", statusCategory.has("key") ? statusCategory.get("key").asText("") : "");
        statusOut.put(
            "colorName",
    ...
    priorityOut.put(
        "name",
        (priority != null && priority.isObject() && priority.has("name"))
    ...
    issueTypeOut.put(
        "name",
        (issueType != null && issueType.isObject() && issueType.has("name"))
    ...
    if (assignee != null && assignee.isObject()) {
      ...
      assigneeOut.put(
          "displayName", assignee.has("displayName") ? assignee.get("displayName").asText("") : "");
    ...
    if (reporter != null && reporter.isObject()) {
      ...
      reporterOut.put(
          "displayName", reporter.has("displayName") ? reporter.get("displayName").asText("") : "");
    ...
    if (commentWrapper != null && commentWrapper.isObject()) {
      JsonNode commentsList =
          commentWrapper.has("comments") ? commentWrapper.get("comments") : null;
