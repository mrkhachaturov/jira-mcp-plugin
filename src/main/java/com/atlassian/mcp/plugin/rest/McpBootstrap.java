 ...
 * HttpServlet} for the wrapper servlet ({@link McpServletWrapper}) to delegate into.
 *
 * <p>This replaces the hand-rolled {@code JsonRpcHandler} dispatch and {@code McpResource} JAX-RS
 * endpoint with the official MCP Java SDK (2.0.0-M2). Tools are adapted to {@code
 * SyncToolSpecification} via {@link McpToolAdapter} and registered on the sync server at build
 * time.
 *
 * <p>Smoke-test wiring registers two read-only tools (Task 2). Task 3 expands the registration to
 * the full 49-tool list and adds resource specifications.
 */
@Named("mcpBootstrap")
public class McpBootstrap {
  ...
  /**
   * Official Jira Software wordmark, used as the server-level icon. The SVG and base64 data-URI
   * live in {@link IconConstants} so they can be reused as per-tool icons on UI-linked tools (F-12
   * / SEP-973).
   */
  ...
  public synchronized HttpServlet buildTransport() {
    ...
    log.info(
        "[MCP] SDK transport built ({} tools, {} resources, {} resource templates, {} completions)",
  ...
  /**
   * Builds the SDK-side Origin allowlist that mirrors the (now-deleted) {@code
   * OriginValidationFilter}: Jira's own base URL, known MCP client hosts, and loopback. The {@link
   * DefaultServerTransportSecurityValidator} returns 403 on rejection (matching the previous
   * filter's behaviour) and treats a missing Origin header as allowed (non-browser clients like
   * curl / MCP CLI tools).
   *
   * <p>Host validation is intentionally left off (empty {@code allowedHosts}) — the filter chain
   * never inspected Host either, and Jira sits behind a reverse proxy where the Host header is
   * whatever the deployer configured.
   */
  ...
  /**
   * Reads the Jira base URL the same way {@code OriginValidationFilter} did: admin override wins,
   * falling back to SAL's {@link ApplicationProperties#getBaseUrl()}.
   */
  ...
  /**
   * Reduces a full URL to its origin form ({@code scheme://host[:port]}) for use with {@link
   * DefaultServerTransportSecurityValidator}, which compares the entire Origin header verbatim.
   */
