...
/**
 * Enforces the security invariant: an {@code MCP-Session-Id} issued to one Jira user may not be
 * used by another. Cross-user replay returns 403; unknown or expired sessions on non-initialize
 * POSTs return 401.
 *
 * <p>Captures the session id from the {@code initialize} response and binds it to the authenticated
 * Jira user. DELETE removes the binding on success.
 */
@UnrestrictedAccess
...
public class SessionBindingFilter implements Filter {
  ...
  private static final ConcurrentHashMap<String, SessionBinding> BINDINGS =
  ...
  public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
    ...
    if (!isInitialize && incomingSid != null) {
      ...
      if (!b.username().equals(currentUser)) {
        log.warn(
            "[MCP-SEC] session-user binding violation: session bound to {} but request from {}",
            b.username(),
            currentUser);
  ...
  private void evictIfFull() {
    ...
    if (BINDINGS.size() >= MAX_BINDINGS) {
      BINDINGS.entrySet().stream()
          .min(
              (a, b) ->
                  Long.compare(a.getValue().createdAtMillis(), b.getValue().createdAtMillis()))
