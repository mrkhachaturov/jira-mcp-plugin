 ...
 *
 * <p>Regex-based approach with code-block protection (extract → transform → restore). Both methods
 * are null/blank-safe and idempotent on non-markup input.
 */
public final class JiraMarkupConverter {
  ...
  private static final Set<String> VALID_JIRA_LANGUAGES =
      Set.of(
          "actionscript",
          "actionscript3",
          "ada",
  ...
  private static final Map<String, String> LANGUAGE_MAPPING =
      Map.ofEntries(
          Map.entry("dockerfile", "bash"),
          Map.entry("docker", "bash"),
  ...
  private static String extractBlocks(
      String text,
      Pattern pattern,
  ...
  /**
   * Convert Jira wiki markup to Markdown. Mirrors upstream {@code
   * JiraPreprocessor.jira_to_markdown()}.
   */
  public static String jiraToMarkdown(String input) {
    ...
    // ── Block quotes ──
    output =
        Pattern.compile("^bq\\.(.*?)$", Pattern.MULTILINE).matcher(output).replaceAll("> $1\n");
    ...
    output =
        Pattern.compile("^((?:#|-|\\+|\\*)+) (.*)$", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(m -> convertJiraListToMarkdown(m.group(1), m.group(2)));

    // ── Headers ──
    // h1. Title → # Title
    output =
        Pattern.compile("^h([0-6])\\.(.*)$", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(m -> "#".repeat(Integer.parseInt(m.group(1))) + m.group(2));

    // ── Citation ??text?? → <cite>text</cite> ──
    output =
        Pattern.compile("\\?\\?([^?]+(?:\\?[^?]+)*)\\?\\?")
            .matcher(output)
            .replaceAll("<cite>$1</cite>");

    // ── Inserted text +text+ → <ins>text</ins> ──
    output = Pattern.compile("\\+([^+]*)\\+").matcher(output).replaceAll("<ins>$1</ins>");

    // ── Superscript ^text^ → <sup>text</sup> ──
    output = Pattern.compile("\\^([^^]*)\\^").matcher(output).replaceAll("<sup>$1</sup>");

    // ── Subscript ~text~ → <sub>text</sub> ──
    output = Pattern.compile("~([^~]*)~").matcher(output).replaceAll("<sub>$1</sub>");

    // ── Strikethrough -text- → -text- (kept as-is in upstream) ──
    output = Pattern.compile("-([^-]*)-").matcher(output).replaceAll("-$1-");

    // ── Quote blocks {quote}...{quote} → > prefixed ──
    output =
        Pattern.compile("\\{quote\\}([\\s\\S]*)\\{quote\\}", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(
                m -> {
                  StringBuilder sb = new StringBuilder();
    ...
    output =
        Pattern.compile("\\{panel(?::([^}]*))?\\}([\\s\\S]*?)\\{panel\\}", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(m -> convertPanel(m.group(1), m.group(2)));
    ...
    output = Pattern.compile("!([^\\n\\s!]+)!").matcher(output).replaceAll("![]($1)");

    // ── Links ──
    // [text|url] → [text](url)
    output = Pattern.compile("\\[([^|]+)\\|(.+?)\\]").matcher(output).replaceAll("[$1]($2)");
    // [url] (bare) → url
    output = Pattern.compile("\\[(.+?)\\]([^(])").matcher(output).replaceAll("$1$2");
    ...
    output =
        Pattern.compile("\\{color:([^}]+)\\}([\\s\\S]*?)\\{color\\}", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll("<span style=\"color:$1\">$2</span>");
  ...
  /**
   * Convert Markdown to Jira wiki markup. Mirrors upstream {@code
   * JiraPreprocessor.markdown_to_jira()}.
   */
  public static String markdownToJira(String input) {
    ...
    output =
        Pattern.compile("^(.*?)\\n([=-])+$", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(m -> "h" + (m.group(2).charAt(0) == '=' ? 1 : 2) + ". " + m.group(1));

    // ── Headers with # prefix (ATX) — require space after # ──
    output =
        Pattern.compile("^(#+) (.*)$", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll(m -> "h" + m.group(1).length() + ". " + m.group(2));
    ...
    output =
        Pattern.compile(
                "<span style=\"color:(#[^\"]+)\">" + "([\\s\\S]*?)</span>", Pattern.MULTILINE)
            .matcher(output)
            .replaceAll("{color:$1}$2{color}");
    ...
    output = Pattern.compile("^> (.*)$", Pattern.MULTILINE).matcher(output).replaceAll("bq. $1");

    // ── User mentions @username → [~username] ──
    output = Pattern.compile("@(\\w+)").matcher(output).replaceAll("[~$1]");
    ...
    for (int i = 0; i < tableLines.length; i++) {
      ...
      if (i < tableLines.length - 1 && Pattern.matches("\\|[-\\s|]+\\|", tableLines[i + 1])) {
