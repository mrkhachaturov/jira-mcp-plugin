...
public class CimdValidatorTest {
  ...
  public void cacheStaysBounded() {
    for (int i = 0; i < 5000; i++) {
      try {
        validator.resolve("https://blocked.invalid.test/" + i);
      } catch (CimdValidator.CimdException expected) {
        // Every URL fails; this test asserts on the cache, not the failures.
      }
    }
