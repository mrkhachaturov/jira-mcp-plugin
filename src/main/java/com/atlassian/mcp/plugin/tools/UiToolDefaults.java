...
/**
 * Shared {@link McpTool#outputSchema()} and {@link McpTool#iconUri()} defaults
 * for the five UI-linked tools whose {@code structuredContent} payload renders
 * as an MCP Apps widget in compatible clients.
 *
 * <p>The structuredContent shape is produced by
 * {@code com.atlassian.mcp.plugin.ResourceContextBuilder} and is identical
 * across the five tools — {@code get_issue} returns a one-element {@code issues}
 * array while the others return many. A single schema covers both cases.
 */
public final class UiToolDefaults {
    ...
    private static Map<String, Object> buildIssueListSchema() {
        ...
        props.put("currentUser", obj(Map.of(
                "name", typed("string"),
