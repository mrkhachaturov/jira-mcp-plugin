...
/**
 * Pulls the Authorization header and the resolved Jira username off each request
 * and stashes them into the per-request {@link McpTransportContext}. Tool handlers
 * (and adapter glue in {@link McpToolAdapter}) read these via
 * {@code exchange.transportContext().contextAsMap().get("authHeader")} etc.
 *
 * <p>This is the principal extraction point. Session-user binding enforcement
 * lives in {@code SessionBindingFilter}, not here.
 */
