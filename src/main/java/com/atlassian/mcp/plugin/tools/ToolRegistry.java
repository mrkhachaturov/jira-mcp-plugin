...
public class ToolRegistry {
  ...
  /**
   * Returns tools visible to the given user, filtered by: - capability (required plugin installed)
   * - admin disabled list - read-only mode (hides write tools)
   */
  ...
  /**
   * Check if a tool is callable by the given user right now. Returns an error message if not, null
   * if OK.
   */
  public String checkToolAccess(String toolName, String userKey) {
    ...
    if (!isCapabilityMet(tool)) {
      return "Tool '"
          + toolName
          + "' requires a Jira product/app that is not installed on this instance";
    }
   ...
   *
   * <ul>
   *   <li>required Jira plugin must be enabled (capability gate);
   *   <li>tool must not be in the admin's disabled list;
   *   <li>tool must not be a write tool when read-only mode is on.
   * </ul>
   */
  public List<io.modelcontextprotocol.server.McpServerFeatures.SyncToolSpecification>
      toSpecifications() {
